...
public:
  SecureSubscriber() : Node("secure_subscriber") {
    ...
    load_public_key(share_dir + "/keys/publisher_node/public.pem");
    load_private_key(share_dir + "/keys/subscriber_node/private.pem");
  }
...
private:
  ...
  void verify_callback(const custom_msgs::msg::SignedData::SharedPtr msg) {
    ...
    // RSA-decrypt AES key using subscriber's private key
    ...
    // Hash decrypted data using SHA-256
